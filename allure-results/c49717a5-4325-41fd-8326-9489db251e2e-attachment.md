...
- Name: tests\inventory\inventory.spec.ts >> Sort products dropdown is visible and selectable
...
```ts
  ...
  27 | });
  28 | 
  29 | test('Sort products dropdown is visible and selectable', async ({ page }) => {
  30 |   // Wait for inventory page to fully load
  31 |   await expect(page).toHaveURL(/inventory/);
  32 | 
  33 |   const dropdown = page.locator('[data-test="product_sort_container"]');
  34 | 
  35 |   // Ensure it's visible before interacting
> 36 |   await expect(dropdown).toBeVisible();
     |                          ^ Error: expect(locator).toBeVisible() failed
  37 | 
  38 |   // Click first (forces readiness)
  39 |   await dropdown.click();
  40 | 
  41 |   // Select option safely
  42 |   await dropdown.selectOption({ label: 'Name (A to Z)' });
  43 | 
  44 |   // Verify dropdown still exists (basic assertion)
  45 |   await expect(dropdown).toBeVisible();
  46 | });
```