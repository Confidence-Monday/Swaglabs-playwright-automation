...
- Name: tests\inventory\inventory.spec.ts >> Sort products by name (A to Z)
- Location: tests\inventory\inventory.spec.ts:29:5
...
```ts
  ...
  12 | });
  13 | 
  14 | test('Inventory items are displayed', async ({ page }) => {
  15 |   const items = page.locator('.inventory_item');
  16 | 
  17 |   await expect(items).toHaveCount(6);
  18 | });
  19 | 
  20 | test('Add item button changes to remove', async ({ page }) => {
  21 |   const addBtn = page.locator('[data-test="add-to-cart-sauce-labs-backpack"]');
  22 | 
  23 |   await addBtn.click();
  24 | 
  25 |   const removeBtn = page.locator('[data-test="remove-sauce-labs-backpack"]');
  26 |   await expect(removeBtn).toBeVisible();
  27 | });
  28 | 
  29 | test('Sort products by name (A to Z)', async ({ page }) => {
> 30 |   await page.selectOption('[data-test="product_sort_container"]', 'az');
     |              ^ Error: page.selectOption: Test timeout of 30000ms exceeded.
  31 | 
  32 |   const firstItem = page.locator('.inventory_item_name').first();
